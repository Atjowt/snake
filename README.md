...
3. Run `sh start.sh [IP] [PORT]` to start the client with the server running in the background **OR** run `snake-server/bin/snake-server` and `snake-client/bin/snake-client` manually as separate instances.
